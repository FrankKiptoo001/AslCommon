...
codeunit 60999 "Webportals"
{
    var
        ...
        PRN: Page "PROC-Internal Requisitions";
        rfqHeader: Record "PROC-Purchase Quote Header";
        purchHeadPage: Page "Proc-Purchase Quote";
    ...
    procedure CreateVendorRFQquoteLines(quoteNum: Text; rfqNum: Text; itemNum: Text; UnitsOfMeasure: Text; Quantityz: Decimal; estmatedUnitCost: Decimal) Msg: Text
    begin
        PurchaseLines.INIT;
        ...
        PurchaseLines."Document No." := quoteNum;
  ...
  EmployeeCard."Job Title" + '::' + EmployeeCard."Company E-Mail" + '::' + FORMAT(EmployeeCard.Title) + '::' + FORMAT(EmployeeCard."Date Of Birth");
    ...
    procedure SubmitPurchaseLine(DocumentType: Integer; DocumentNo: Text; FunctionNo: Code[50]; LocationID: Text; ExpectedDate: Date; FunctionDesc: Text; UnitsOfMeasure: Text; Quantityz: Decimal; estmatedUnitCost: Decimal)
    begin
        PurchaseLines.INIT;
        PurchaseLines."Document Type" := PurchaseLines."Document Type"::Quote;
        if DocumentType = 1 then
    ...
    begin
        ...
        THEN BEGIN
            if ApprovalsMgmt.IsPurchaseApprovalsWorkflowEnabled(PurchaseRN) = true then
                ...
                Error('Approval Workflow not set!');
        END;
    end;
    ...
    begin
        ...
        THEN BEGIN
            if ApprovalMgmtFin.IsSRNEnabled(StoreRequisition) = true then
                ...
                Error('No approval workflow set up!');
        END;
    end;
    ...
    procedure StoresRequisitionCreate(EmployeeNo: Text; UserID: Text; RequiredDate: Date; Purpose: Text; Department: Code[20]; Campus: Code[20]; DepartmentName: Text[250]; CampusName: Text[250]; ReqType: Option; ResponsibilityCentre: Code[10]; IssueStore: Code[30]) LastNum: Text
    begin
    ...
    begin
        ...
        if StoreRequisition.Find('-') then begin
            ...
            if AType = 0 then StoreReqLines.Type := StoreReqLines.Type::Item;
            if AType = 1 then StoreReqLines.Type := StoreReqLines.Type::"Minor Asset";
            StoreReqLines.Validate("Requistion No");
            StoreReqLines."No." := ItemNo;
            StoreReqLines.Validate("No.");
    ...
    procedure SubmitImprestSurrHeader(ImprestNo: Text) Message: Text
    begin
        ...
        IF NOT ImprestSurrHeader.FIND('-') THEN BEGIN
            NextJobapplicationNo := NoSeriesMgt.GetNextNo('SURRENDER', 0D, TRUE);
            ImprestRequisition.RESET;
    ...
    procedure SubmitMemoHeader(MemoNo: Text; Username: Text; UserID: Text; responsibilityCenter: Text) Message: Text
    begin

        //if ApprovalMgmtFin.IsImprestEnabled(ImprestRequisition) = false then Error('No approval workflow set for imprest');
        ImprestRequisition.RESET;
        ...
        ImprestRequisition.SETRANGE("Memo Number", MemoNo); //new
        ...
        IF NOT ImprestRequisition.FIND('-') THEN BEGIN
            ...
            /*Copy the details to the user interface*/
            ImprestRequisition.INIT;
            NextLeaveApplicationNo := NoSeriesMgt.GetNextNo('IMP', 0D, TRUE);
            ImprestRequisition."No." := NextLeaveApplicationNo;
            ImprestRequisition."Memo Number" := MemoNo;
            ImprestRequisition.Date := TODAY;
            ...
            ImprestRequisition."Global Dimension 1 Code" := 'KNQA';
            ImprestRequisition.Status := ImprestRequisition.Status::Pending;
            ImprestRequisition."Shortcut Dimension 2 Code" := MemoImprestHead."Department Code";
            ImprestRequisition."Shortcut Dimension 3 Code" := MemoImprestHead."Activity Code";
            //ImprestRequisition."Function Name" := MemoImprestHead."Programme Name";
            ImprestRequisition."Budget Center Name" := MemoImprestHead."Department Name";
            ImprestRequisition."No. Series" := 'IMP';
    ...
    begin
        ...
        if tblTenders.Find('-') then begin
            ...
            IF NOT tblTenderBids.FIND('-') THEN BEGIN
                ...
                tblTenderBids."Expected Opening Date" := tblTenders."Expected Opening Date";
                tblTenderBids."Expected Closing Date" := tblTenders."Expected Closing Date";
                tblTenderBids."Posting Date" := Today;
                tblTenderBids."Posting Description" := tblTenders."Posting Description";
                tblTenderBids."Responsibility Center" := tblTenders."Responsibility Center";
